...
The authors also used the `lme4` package in R to run a mixed effects model to determine if sentiment changes over time and differs between teachers and non-teacher.
...
```{r}
ngss_tweets_q1 <- search_tweets(q = "#NGSSchat", n=5000)
```
...
```{r}
ngss_tweets_q3 <- search_tweets2(c("#NGSSchat OR ngss",
...
To do so, we'll need to add some additional search terms to our list. Run the following code in your console:
...
First, it's likely the authors removed retweets from their analysis since a retweet is simply a user reposting someone else's tweet and would duplicate the exact same content of the original. It's also likely that they limited their analysis to just English language tweets.
...
Now let's use the `select()` function select the following columns from our new `ngss_teeets_en` data frame:
