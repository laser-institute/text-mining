...
a.  **Import & Subset Tweets**. In this section, we introduce new functions from the `rtweet` package to search for and filter tweets and users of interest.
...
As you are probably already aware, we have way more data than we need for our analysis and need to pare it down quite a bit.
...
Often in text analysis, we will want to remove these stop words if are not useful for an analysis. The `stop_words` dataset in the tidytext package contains stop words from three lexicons. We can use them all together, as we have here, or [`filter()`](https://dplyr.tidyverse.org/reference/filter.html) to only use one set of stop words if that is more appropriate for a certain analysis.

Let's take a closer like
