...
The Text Mining (TM) Learning Labs will provide LASER Institute scholars hands-on experience with popular techniques for collecting, processing, and analyzing text-based data---including mining data from application programming interfaces or APIs, screen-scraping, sentiment analysis, topic modeling, text classification and more.

Our first lab is guided by a recent publication by LASER Institute presenter Josh Rosenberg's recent article, *Advancing new methods for understanding public sentiment about educational reforms* (Rosenberg et al., 2021), and is divided into the following four parts:
