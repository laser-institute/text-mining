...
Click the green arrow in the right corner of the "code chunk" that follows to load the {tidyverse} library as well as the {here} package introduce in previous labs.
...
Let's use the by now familiar `read_csv()` and `here()` functions to import our `ss_tidy_tweets.csv` data from Learning Lab 1 that we'll be using for our sentiment analysis:
