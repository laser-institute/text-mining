...
To prepare for our analyses, first we need to rebuild the `tweets` dataset from my `ngss_tweets` and `ccss_tweets` and select both the `id` that is unique to each tweet, and the `text` column which contains the actual post:
