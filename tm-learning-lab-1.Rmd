...
-   **Learning Lab 2: Bigrams + Word Pairs = 2x the Fun.** For our second lab, we'll see what insights bigrams, word pairs and word correlations tell us about our tweets and what they might suggest about public sentiment towards the CCSS and NGSS.
...
If you'd like to use the data we've been working with for your reach, see if you can make a more meaningful and attractive word cloud for each set of standards. For example, words like "core" and "common" are not very helpful since those were in our search terms when pulling data from Twitter. See if you can filter those out before making a word cloud and run `?wordcloud2` in your console to take a look at the settings for tweaking the appearance of your word cloud.

Use a separate r script file, try modifying our list of stop words, retidying our text, and doing a new word count with these and perhaps other words removed. Also, take a look at the help file for `wordclouds2` to see if there might be otherwise you could visually improve this visualization.
