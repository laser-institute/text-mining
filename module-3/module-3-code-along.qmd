...
- **Fit a Topic Modeling with LDA**.We will learn to use the
  `topicmodels` package and associated `LDA()` function for unsupervised
  classification of our forum discussions to find natural groupings of
  words, or topics.
- **Choose K.** We will take an introductory look at fitting our LDA to
  K, the number of topics within the text.

::: notes
Just as a heads up, this will be a more computationally-intensive
module. We'll have plenty of time to discuss and ask questions as our
IDEs build the LDA models!
:::
...
- [Latent Dirichlet allocation
  ...
  algorithms (via `LDA()` in R) expects a document-term matrix (DTM) as
  the data input.
...
- NOTE: This is computationally intensive, so don't panic if R seems to
  take a long time to create the new `forums_lda` object (or even
  freeze!).
...
- Adjust the word number argument in `terms()` to make the word list 10
  per topic instead.
...
- This is just an initial approach. After running this against another
  algorithm that uses the `stm` package, 14 looks to be a better K for
  this LDA.
...
![](images/clipboard-1958206973.png){width="1200"}
:::
...
After running this with another function in the `stm` package (in the
Case Study), we've identified 14 as a new optimal number.
:::
