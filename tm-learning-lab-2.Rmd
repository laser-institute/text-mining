...
On to the Dark Side. The {vader} package basically has just one function, `vader_df()` that does one thing and expects just one column from a data frame. Let's give VADER the our `ccss_sample` data frame and include the `$` operator to give it just our `text` column containing our tweets.
...
But what if we wanted to compare these more easily to our other sentiment lexicons just to check if result are fairly consistent?
...
Not quite as bleak as we might have expected according to VADER!
