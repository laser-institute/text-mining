---
title: "Working with Sentiment Lexicons"
...
::: notes
-   The data that we're working with for today's Code Along has already
...
::: notes
-   The *explore* phase of the learning analytics workflow is to gain a
    good intuition about the underlying data through summary statistics,
    visualizations, and, if need be, additional wrangling
...
sentiment of the views of multiple stakeholders toward them.

To establish how public sentiment about this reform might be similar to
or different from past efforts, we applied a suite of data science
techniques to posts about the standards on Twitter from 2010-2020 (N =
571,378) from 87,719 users.

Applying data science techniques to identify teachers and to estimate
tweet sentiment, we found that the public sentiment towards the NGSS is
overwhelmingly positive---33 times more so than for the CCSS.

Mixed effects models indicated that sentiment became more positive over
time and that teachers, in particular, showed a more positive sentiment
towards the NGSS. We discuss implications for educational reform efforts
and the use of data science methods for understanding their
implementation.
:::
...
::: notes
These should already be installed in RStudio, so there's no need to use
...
\[After all code is run\] There we go, now we can see that CCSS scores
negative, while NGSS is overall positive.
:::
...
the `value` column. This is for AFINN:
...
Calculate a single sentiment score for NGSS and CCSS using the NRC
lexicon.
...
::: notes
\[Tab 1\] Now let's calculate a quick score for using the `afinn`
lexicon. Recall that AFINN provides a value from -5 to 5 for each.

\[Tab 3\] The `nrc` lexicon contains "positive" and "negative" values
just like bing, but also includes values like "trust" and "sadness" as
shown below. If you use nrc, you will need to use the filter() function
to select rows that only contain "positive" and "negative."
:::
...
-   Complete the *Explore* part of the [case
...
-   Complete the [Sentiment Analysis
    badge](https://laser-institute.github.io//text-mining/module-2/tm-2-badge.html)
-   Do [essential readings for the next
    module](https://laser-institute.github.io//text-mining/module-3/tm-3-readings.html)
:::
